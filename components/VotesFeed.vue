<template>
  <div class="votesFeed">
    ...
    <div class="cards row">
      <div v-for="(vote, index) of getVotes" :key="index" class="card" @click="openUrl(vote.requestId)">
      ...
      <my-button v-if="visibleItems < 25" class="showMore" @click="visibleItems += 10">
        Show more
      </my-button>
    </div>
  </div>
...
export default {
  ...
  async created () {
    ...
    for (let index = 0; index < this.votes.length; index++) {
      ...
      for (const [key, value] of Object.entries(this.votes[index].votes)) {
