  ...
  <div class="maptts" style="text-align: center">
    ...
    <div class="content">
      ...
      >
        ...
        lohl. Should fetch from a lot more players now
      </Message>
      ...
      <div class="row flex justify-between">
        <div class="search">
          <div class="p-inputgroup">
            <InputText v-model="search" placeholder="Search" @keyup.enter="startSearch" />
            <Button icon="pi pi-search" @click="startSearch" />
          </div>
        </div>

        <div class="flex">
          <Button icon="pi pi-angle-left" :disabled="loading || page <= 0" @click="page--" />
          <p class="self-center py-3 px-5" style="background: #141417">
            {{ page }}
          </p>
          <Button icon="pi pi-angle-right" :disabled="loading || scores.length < 25" @click="page++" />
        </div>
      </div>
      ...
      <div class="row justify-center">
        <div class="flex">
          <Button icon="pi pi-angle-left" :disabled="page <= 0" @click="page--" />
          <p class="self-center py-3 px-5" style="background: #141417">
            {{ page }}
          </p>
          <Button icon="pi pi-angle-right" :disabled="scores.length < 25" @click="page++" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    doesHttpOnlyCookieExist (cookiename) {
      ...
      if (!document.cookie.includes(cookiename + '=')) { return true } else { return false }
    },
    openUrl (id) {
