  ...
  <div class="tierlist">
    <sub-header title="mEBSMBSBSMTM (fr)">
      <h3 class="text-lg text-center mt-4">
        also <strong>famously</strong> known as muffn's Epic Beat Saber Mappers Beat Saver Beat Saber Maps
        Tierlist Maker by muffn
      </h3>
    ...
    <div class="content flex flex-col justify-center pt-16 w-full">
      <Message class="self-center mt-8" :closable="false">
        <p>Search beat saver down below to get maps and drag & drop them into a tier</p>
        <p>Just screenshot with win + shift + s <strong>:D</strong></p>
        <p>
          Also zoom out with ctrl + mouse wheel if you dont have enough space. Couldnt figure out a quick good
          way to make a screenshot of the tierlist yet
        </p>
      </Message>

      <div class="w-full flex flex-col py-10 mt-4 xl:px-20">
        <p>Cover size</p>
        <Slider v-model="coverSize" class="w-1/4 my-4" :min="100" :max="300" :tooltips="false" />

        <div class="flex w-full justify-center">
          <InputText
            placeholder="Enter Title"
            class="mapperName w-full text-center"
            style="border-bottom: solid 2px; padding-bottom: 10px; border-radius: 0"
          />
        </div>

        <div class="tiers flex flex-col">
          <div v-for="(tier, index) in tiers" :key="tier.key" class="tier">
            <div class="flex" style="min-height: 100px">
              ...
              <InputText
                ...
                :value="tier.label"
              />
              <draggable-maps v-model="tier.list" :cover-size="coverSize" @onRightClick="onRightClick" />
            </div>
          </div>
        </div>
        ...
        <div v-if="maps.length" class="flex flex-col mx-4">
          ...
          <draggable-maps v-model="maps" :cover-size="coverSize" class="mb-4" @onRightClick="onRightClick" />
          <Button
            v-if="pagination > 0"
            class="self-center"
            label="MOOORE"
      ...
      <OverlayPanel ref="tierMenu">
        <div class="flex flex-col">
          <div class="flex justify-between mt-2">
            <label class="font-bold" for="bgcolor">Background Color: </label>
            <ColorPicker
              v-if="editTier != null"
              id="bgcolor"
          ...
          <div class="flex justify-between mt-2 mb-4">
            <label class="font-bold" for="textcolor">Text Color: </label>
            <ColorPicker
              v-if="editTier != null"
              id="textcolor"
          ...
          <Button label="Delete" class="p-button-danger" icon="pi pi-trash" @click="deleteTier" />
        </div>
      </OverlayPanel>
    ...
    onRightClick (event, map) {
      this.mapContextMenu = [
        ...[
                    ...
                    ...(map.highestStar > 0 ? [`Highest Star rating: ${map.highestStar}`] : [])
        ].map((string) => {
          return {
            label: string,
            disabled: true
