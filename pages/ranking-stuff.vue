<template>
  <div class="rankingStuff">
    <sub-header title="Ranking stuff for scoresaber" />
    ...
    <div class="content">
      ...
      <div class="flex">
        <my-button href="https://muffnlabs.de/static/qualified-maps.bplist">
          Download playlist
        </my-button>
...
export default {
  ...
  computed: {
    passedRankedDateAmount () {
      return this.maps.reduce((a, b) => {
        if (b.hoursleft < 0) { return a + 1 } else { return a }
      }, 0)
    },
    remainingTimeAmount () {
      return this.maps.reduce((a, b) => {
  ...
  async created () {
    this.maps = (await this.$defaultApi.$get('scoresaber/qualified')).sort((a, b) => {
      if (a.hoursleft > 0) { return a.hoursleft - b.hoursleft } else { return b.hoursleft - a.hoursleft }
    })
